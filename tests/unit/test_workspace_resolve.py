...
from substrat.agent import USER, AgentNode, AgentTree
...
def test_resolve_own_scope(tree: AgentTree) -> None:
    ...
    scope, name = resolve(alice, "my-ws", tree)
...
def test_resolve_child_scope(tree: AgentTree) -> None:
    alice = _get(tree, "alice")
    dave = _get(tree, "dave")
    scope, name = resolve(alice, "dave/output", tree)
...
def test_resolve_parent_scope(tree: AgentTree) -> None:
    root = _get(tree, "root")
    alice = _get(tree, "alice")
    scope, name = resolve(alice, "../shared", tree)
...
def test_resolve_parent_scope_from_root(tree: AgentTree) -> None:
    root = _get(tree, "root")
    scope, name = resolve(root, "../shared", tree)
    assert scope == USER
    assert name == "shared"
